...
class Plot():
	...
	def plotPie(self, result):
		...
		#plt.text(-0.7,-1.2,"Detect delay: "+str(delay)+" days\nCoop: "+str(result.scenario.provider_participation)+"% MOs, "+str(result.scenario.intermidiaries_participation)+"% VoIP Carriers",fontsize=11)
	...
	def plotBars2(self, revenues, case):
		...
		#plt.plot([0., self.scenario.cycles], [threshold,threshold], "k--", color='red')
		#plt.text(self.scenario.cycles-2, 7560+0.01, 'fraud profit threshold', fontsize=12)
	...
	def plotDetectResult(self, blacklists, fraudBehaviour):
		x = [i for i in range(self.scenario.cycles)]
		#detects = [0 for i in range(len(blacklists))]
		#errors = [0 for i in range(len(blacklists))]
		detects = []
		errors = []
		...
		#min_profit_per_cycle = fraudmins_per_fraudster * 0.03
		...
		N = len(blacklists)
		ind = np.arange(N)  # the x locations for the groups. 'yellowgreen', 'gold', 'lightcoral','crimson',
		...
		#plt.title('blacklist statistics: frauds: '+str(self.scenario.frauds_percentage)+'%, fraudsters: '+str(self.scenario.fraudsters_percentage)+'%')
